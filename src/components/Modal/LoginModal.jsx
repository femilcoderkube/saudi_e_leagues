...
const LoginModal = () => {
  ...
  const [visible, setVisible] = useState(false); // Controls Forgot Password view
  ...
  // Clear state after showing success/error
  ...
  const handleForgotSubmit = async (values, { setSubmitting }) => {
    try {
      // Replace with your actual API call for forgot password
      await dispatch(resetPassword(values?.email)).unwrap();
      toast.success(t("auth.forgot_email_sent"));
      setVisible(false); // Return to login form after submission
    } catch (error) {
