...
const MatchFound = ({ data }) => {
  ...
  if (data.extras?.tempId) {
    subject =
      i18n.language == "en"
        ? data.notificationId.Subject.toString().replace(
            "{#1}",
            data.extras.tempId
          )
        : data.notificationId.SubjectAr.toString().replace(
            "{#1}",
            data.extras.tempId
          );
    body =
    ...
    body =
      i18n.language == "en"
        ? data.notificationId.Body.toString()
        : data.notificationId.BodyAr.toString();
  }
  let imageUrl =
    data?.notificationId?.notificationtype === 4
      ? data?.extras?.leagueLogo
        ...
        <div className="notification-box-rotate">
          <div className="notification-box-head-wp flex justify-between p-5 border-b border-[#262968]">
            <div className="notification-box-head flex items-center gap-4">
              ...
              />
              <h6 className="text-xl sleading-6">
                {data?.notificationId?.notificationtype === 4
                  ? notificationData.leaguename
