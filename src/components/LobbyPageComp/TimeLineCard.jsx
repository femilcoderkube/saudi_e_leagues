...
const TimelineCard = () => {
  ...
  const allStepsCompleted = leagueData?.timeLine?.every(step => new Date(step.endDate) < new Date());
  ...
  return (
    <div className="timeline-card rounded-xl overflow-hidden bg-[#0E123A] text-white">
      <div className="timeline-card__header flex items-center gap-3 px-4 py-[1.4rem] border-b border-[#2A2F64]">
        <img className="sm:w-[1.75rem] w-[1.5rem]" src={timelineClock} alt="clock" />
        <h3 className="timeline-card__title text-base sm:text-xl font_oswald ">Tournament Timeline</h3>
      </div>
      <div className="timeline_container custom_scroll">
        <div className={`timeline-card__steps p-4 ltr:pr-18 rtl:pl-18 flex flex-col gap-4 sd_before relative ${allStepsCompleted ? "completed" : "disabled"}`}>
