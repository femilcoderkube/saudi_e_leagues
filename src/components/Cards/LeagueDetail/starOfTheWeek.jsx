...
import { formatAmountWithCommas, getRandomColor, getServerURL } from "../../../utils/constant";

import { Swiper, SwiperSlide } from "swiper/react";
import "swiper/css";
import "swiper/css/navigation";
import { Pagination } from "swiper/modules";
import ScoreTicker from "../../LeagueDetail/Score_ticker";
          ...
          {starOfTheWeekData.map((star, index) => {
            ...
            return (
              <SwiperSlide className="w-full" key={index}>
                <div className="mob-star-week sm:max-w-[32rem] sm:w-full flex flex-col bg-no-repeat bg-center bg-cover relative p-5 mx-auto md:order-2 order-2">
                  <div className="sd_bedge_left-con border-b-1 pb-5 mb-6 flex flex-row items-center justify-between gap-4 w-full">
                    <div className="sd_bedge-lable flex gap-2 items-end">
                      {i18n.language === "ar" ? (
                        <img
                          className="shining-star-arabic"
                          src={IMAGES.mob_star_of_week_arabic}
                          alt=""
                          style={{ width: "10rem" }}
                        />
                      ) : (
                        <img
                          ...
                          src={IMAGES.mob_star_of_week}
                          alt=""
                          style={{ width: "12.35rem" }}
                        />
                      )}
                      <span className="text-sm opacity-70">
                        ({t("star_of_the_week.week")} {star.weekNumber})
                      </span>
                    </div>
                  ...
                  <div
                    className={`ltr:text-right rtl:text-left pr-2 ${
                      isYourPoints ? "" : "mb-4"
                    }`}
                  >
                    {isYourPoints && (
                      <span className="text-sm purple_col">
                        {t("star_of_the_week.your_points")}:{" "}
                        {star?.requestedUsersScore?.weeklyScore < 0
                          ? 0
                          : star?.requestedUsersScore?.weeklyScore}
                      </span>
                    )}
                  </div>
