...
const Sidebar = ({ onItemClick }) => {
  ...
  const items = [
    { name: "Saudi eLeague", path: "/", src: bolt, hoverSrc: bolt_hover },
    { name: "Prime", path: "/item1", src: prime_icon, hoverSrc: prime_hover },
  ...
  return (
    <aside className="w-[22.5rem] text-white h-full overflow-hidden">

      <Link
        to={"/admin-control/partners"}
        className="sd_logo">
        <img src={asideLogo} alt="" className="max-w-[initial] object-cover w-full" />
      </Link>
      
      <div className="aside_bottom-block w-[20rem]">
        ...
        {items.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                <div className={`aside_link-con relative ${isActive ? "active" : ""}`}>
                  <NavLink
                    to={item.path}
                  ...
                  >
                    <img
                      src={(isHovered || isActive) && item.hoverSrc ? item.hoverSrc : item.src}
                      style={{width:'2.5rem'}}
                      alt=""
                    />
        ...
        <Link
          ...
          <span className="ml-2 px-6 py-3 block text-xl font_oswald purple_col font-medium">Contact Us</span>
        </Link>
        <Link
          to={"/admin-control/partners"}
          className="block hover:opacity-70 duration-400">
          <span className="ml-2 px-6 py-3 block text-xl font_oswald purple_col font-medium">Privacy Policy</span>
        </Link>

        {/* === Social Links Block Start === */}
        <div className="sd_social--block flex flex-col mt-[6rem] border-b border-[#131645] pb-6">
          <h2 className="text-[2rem] font_oswald max-w-full px-8 tracking-wide uppercase">Follow Us</h2>
          <ul className="sd_social--links flex gap-4 justify-center self-center flex-wrap ">
            {social_links.map(({ name, path, Icon }) => (
              <li key={name}>
                <NavLink to={path} className="inline-block p-[0.375rem] rounded-xl hover:opacity-70 duration-400 sd_radial-bg">
                  <Icon />
                </NavLink>
              </li>
            ))}
