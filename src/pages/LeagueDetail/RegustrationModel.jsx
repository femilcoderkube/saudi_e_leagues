...
function RegistrationModel({
  ...
  const HandleOnSubmit = (e) => {
    ...
    socket.emit(SOCKET.LEAGUEJOIN, { Lid: lId , userId: user._id,gameId : inputId });
              ...
              <div className="match_reg--popup relative sd_before sd_after">
                  <div className="popup_header px-8 pt-8 pb-5 flex items-start justify-between mt-3 text-center sm:mt-0 sm:text-left">
                      <img src={match_reg} alt="" style={{width:'10rem'}} />
                        <button type="button" onClick={ ()=> setRegistrationModal(false)} className="pt-2 cursor-pointer">
                  ...
                  <div className="popup_body px-4 py-3 ">                   
                      <div className="text-center w-full">
                          <input type="text" id="inputID" className="sd_custom-input px-4 text-xl focus:outline-0 focus:shadow-none leading-none text-[#7B7ED0]" placeholder="Input ID" required 
                          onChange={(e) => {
                            e.preventDefault();
                  ...
                  <div className="popup_footer px-12 pt-6">
                    <Link
                      onClick={()=> HandleOnSubmit()} className='popup_submit-btn text-xl uppercase purple_col font-medium font_oswald hover:opacity-70 duration-400'>
                      Input ID
                    </Link>
                    <Popup_btn />
                  </div>           
